...
### 4. 이미지 로드 시 `net::ERR_QUIC_PROTOCOL_ERROR` 발생

- **증상:**
  - Supabase Storage에 이미지를 성공적으로 업로드했음에도, 브라우저에서 해당 이미지를 불러오지 못하고 네트워크 오류가 발생함.

- **원인:**
  - 코드의 문제가 아닌, 브라우저와 서버 간의 최신 통신 프로토콜(HTTP/3, QUIC) 단계에서 발생하는 네트워크 불안정성 또는 비호환성 문제.
  - 방화벽, 캐시 등 다양한 외부 요인에 의해 발생할 수 있음.

- **해결:**
  - **'캐시 버스팅(Cache Busting)'** 기법을 적용하여 문제를 우회함.
  - 이미지를 렌더링하는 시점에, 이미지 URL의 끝에 `?t=<현재 타임스탬프>`와 같은 고유한 쿼리 파라미터를 덧붙임.
  - 이를 통해 브라우저는 매번 이미지를 새로운 리소스로 인식하여, 문제가 되는 캐시나 네트워크 프로토콜 상태를 거치지 않고 이미지를 깨끗하게 다시 요청하도록 강제함.
  - 이 로직을 `ImageUpload` 컴포넌트와 포트폴리오 이미지 렌더링 부분에 적용하여 해결.
